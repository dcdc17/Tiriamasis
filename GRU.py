...
def forecast_future(model, recent_data, n_future, scaler, target_scaler):
    predictions = []
    input_seq = recent_data[-n_past:]  # Start with the most recent data
    for _ in range(n_future):
        input_scaled = scaler.transform(input_seq.reshape(-1, input_seq.shape[1])).reshape(input_seq.shape)
        pred_scaled = model.predict(input_scaled[np.newaxis, :, :])  # Predict one step
        pred = target_scaler.inverse_transform(pred_scaled)  # Reverse scaling
...
for m, ax in tqdm(zip(metals, axs.flatten())):
    ...
    grid_model.add(GRU(150, return_sequences=True, input_shape=(n_past, len(combined.columns))))
...
handle3, = ax.plot([], [], color='green', label='Forecasted Stock Price')
handles=[handle1, handle2, handle3]
labels=['Real Stock Price', 'Predicted Stock Price', 'Forecasted Stock Price']
fig.legend(handles, labels, loc='upper right', ncol=4)

fig.tight_layout()
plt.savefig("pred/gru_rez2.png")
plt.show()
